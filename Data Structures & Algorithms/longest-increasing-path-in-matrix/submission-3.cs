public class Solution {
    public int LongestIncreasingPath(int[][] matrix) {
        ...
        for (int i = 0; i < matrix.Length; i++) memoGrid[i] = new int[matrix[0].Length];
        ...
        int Dfs(int curRow, int curCol) {
            ...
            if (curRow + 1 >= 0 && curRow + 1 < matrix.Length &&
                matrix[curRow + 1][curCol] > matrix[curRow][curCol])
            ...
            if (curCol + 1 >= 0 && curCol + 1 < matrix[0].Length &&
                matrix[curRow][curCol + 1] > matrix[curRow][curCol])
